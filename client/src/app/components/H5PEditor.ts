...
export class H5PEditorComponent extends HTMLElement {
	...
	public get h5pUrl(): string {
		return this.getAttribute('h5p-url') ?? undefined!;
	}
	...
	public save = async (): Promise<{
		...
		try {
			content = await new Promise<{ library: string; params: string }>((res, rej) => {
	...
	private appendH5pUrl(url: string): string {
		return (this.h5pUrl != undefined ? this.h5pUrl : '') + url;
	}
	...
	private async render(contentId: string): Promise<void> {
		...
		// 	editorModel.integration.editor!.assets.js.map((script) =>
		// 		this.appendH5pUrl(script)
		// 	);
		// editorModel.scripts = editorModel.integration.editor!.assets.js;
		// editorModel.integration.editor!.assets.css =
		// 	editorModel.integration.editor!.assets.css.map((script) =>
		// 		this.appendH5pUrl(script)
		// 	);
		// editorModel.styles = editorModel.integration.editor!.assets.css;

		editorModel.integration.editor!.ajaxPath = this.appendH5pUrl(editorModel.integration.editor!.ajaxPath);
