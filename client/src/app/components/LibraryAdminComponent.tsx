    ...
    return (
      <div>
        ...
        {this.state.libraries === null ? (
          ...
          <div>
            <p>The following libraries are installed in the library storage:</p>
              ...
              <tbody>
                {this.state.libraries?.map((info) => (
                  ...
                  >
                    <tr>
                      <td>
                        {info.title} ({info.majorVersion}.{info.minorVersion}.
                        {info.patchVersion})
                      </td>
                      <td>
                        {info.runnable ? (
                          <input
                            type="checkbox"
                            checked={info.restricted}
                            onChange={() => this.restrict(info)}
                          />
                        ) : null}
                      </td>
                      <td>{info.instancesCount}</td>
                      <td>{info.instancesAsDependencyCount}</td>
                      <td>{info.dependentsCount}</td>
                      <td>
                        <button
                          className="btn btn-info"
                          onClick={() => this.showDetails(info)}
                        >
                          <span
                      ...
                      <td>
                        {info.canBeDeleted ? (
                          <button
                            ...
                            onClick={() => this.deleteLibrary(info)}
                          >
                            <span
                      ...
                      <tr>
                        <td colSpan={7}>
                          <LibraryDetails
                            ...
                            onClose={() => this.closeDetails(info)}
                          />
                        </td>
                      </tr>
                    ) : null}
