...
export class H5PPlayerComponent extends HTMLElement {
	 ...
	 * goes wrong.
	 */
	public get loadContentCallback(): (contentId: string) => Promise<IPlayerModel> {
		return this.privateLoadContentCallback;
	}

	public set loadContentCallback(callback: (contentId: string) => Promise<IPlayerModel>) {
	...
	private privateLoadContentCallback!: (contentId: string) => Promise<IPlayerModel> ;
	private resizeObserver: ResizeObserver | undefined;
	private root!: HTMLElement|null;
	private h5pInstanceInternal!: IH5PInstance;
	private h5pObjectInternal: IH5P|undefined;
	...
	public getCopyrightHtml(): string | undefined {
		...
		if (!metadata) {
			metadata =
				this.playerModel.integration.contents![`cid-${this.playerModel.contentId}`]
					.metadata;
			if (!metadata) {
				return undefined;
			}
